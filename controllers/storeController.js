...
exports.getStore =  (req, res, next) =>{
  ...
  if(undefined == queryTimeStamp){
    const selectKeyQuery = Store.find({"key": key}).sort({timestamp:"desc"}).limit(1).select("value");
      selectKeyQuery.exec()
        .then(result => {
          if(0 == result.length){
            return res.status(200).json({message:"There is no key for : "+ key});
          }
    ...
    const selectWithTimeStampQuery = Store.find(
      {'key': key})
      .where('timestamp').lte(queryTimeStamp)
      .sort({timestamp:"desc"})
      .limit(1)
      .select("value");
    selectWithTimeStampQuery.exec()
      .then(result => {
        if(0 == result.length){
...
exports.saveStrore = (req, res, next) => {
  ...
  if (reqStoreArray.length === 0) return next(new Error('JSON request is empty!'));
 ...
 //Create new Store object if the given key doesn't exist in the DB
  store.save(store, (err, result) => {
    if(err){
      logger.error("Error occured while Transaction : "+ err);
      next(err);
    }
